...
results_path = "C:\Users\feder\OneDrive\Desktop\03 Academics\03 Graduate Education\ROB599\ROB599_AV\Time-Varying Non-Linear MPC\Stability\";
current_fnameNL = results_path + "NL_Cond" + "wind_ramp.mat" + "_Pw15_Hz2";
current_fnameL = results_path + "L_Cond" + "wind_ramp.mat" + "_Pw15_Hz2";
load("increment_index.mat")
load("trajectory.mat")
xHistoryNL = load(current_fnameNL + "_xHistory.mat").xHistory;
% xHistoryL = load(current_fnameNL + "_xHistory.mat").xHistory;
...
for wp_id = 1:length(waypoints)
    waypoint_performance_summary(wp_id, 1) = min(sqrt(sum((xDesired(1:3, index_list(wp_id))' - xHistoryNL(:, 1:3)).^2, 2)));
    waypoint_performance_summary(wp_id, 2) = min(sqrt(sum((xDesired(4:6, index_list(wp_id))' - xHistoryNL(:, 4:6)).^2, 2)));
    % waypoint_performance_summary(wp_id, 3) = min(sqrt(sum((xDesired(1:3, index_list(wp_id))' - xHistoryL(:, 1:3)).^2, 2)));
    % waypoint_performance_summary(wp_id, 4) = min(sqrt(sum((xDesired(4:6, index_list(wp_id))' - xHistoryL(:, 4:6)).^2, 2)));
end
...
%         r = plot3(new_drone_shape_l(1,1:6),new_drone_shape_l(2,1:6),new_drone_shape_l(3,1:6),'b.-');
% 
...
%     switch video_mode
%         case "follow"
%             xlim([min(xHistoryNL(i, 1)-3,xDesired(1, i)-3), max(xHistoryNL(i, 1)+3, xDesired(1, i)+3)])
%             ylim([min(xHistoryNL(i, 2)-3,xDesired(2, i)-3), max(xHistoryNL(i, 2)+3, xDesired(2, i)+3)])
%             zlim([min(xHistoryNL(i, 3)-3,xDesired(3, i)-3), max(xHistoryNL(i, 3)+3, xDesired(3, i)+3)])
%         case "global"
