...
function desiredTrajectory = trajectory_generator(RRTWaypoints, timeConstant, velLimits, accLimits)
    ...
    [intermediatePt,intermediateVel,intermediateAcc,intermediateT] = contopptraj(RRTWaypoints(:, 1:3)', velLimits, accLimits);

    % Determine the RRT waypoints's time and required number of samples
    for idx = 1:size(RRTWaypoints, 1)
        [~ , relevantIndex] = min(pdist2(intermediatePt', RRTWaypoints(idx,1:3)));
    ...
    [desiredPt,desiredVel,qdd,qddd,qdddd,pp,tPoints,tSamples] = minsnappolytraj(RRTWaypoints(:,1:3)', timeWaypoint, nSamples);
